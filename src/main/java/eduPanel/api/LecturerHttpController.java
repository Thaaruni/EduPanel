...
public class LecturerHttpController {
    ...
    public LectureTo createNewLecturer(@ModelAttribute @Validated(LecturerReqTo.Create.class)
                                        LecturerReqTo lecturerReqTo) {
        AppStore.setEntityManager(em);
        LecturerService lecturerService = ServiceFactory.getInstance()
                .getService(ServiceFactory.ServiceType.LECTURER);
    ...
    public void updateLecturerDetailsViaMultipart(@PathVariable("lecturer-id") Integer lecturerId,
                                                  @ModelAttribute @Validated(LecturerReqTo.Update.class)
                                                  LecturerReqTo lecturerReqTo) {
        AppStore.setEntityManager(em);
        LecturerService lecturerService = ServiceFactory.getInstance()
                .getService(ServiceFactory.ServiceType.LECTURER);
    ...
    public void updateLecturerDetailsViaJson(@PathVariable("lecturer-id") Integer lecturerId,
                                             @RequestBody @Validated LectureTo lectureTo) {
        AppStore.setEntityManager(em);
        LecturerService lecturerService = ServiceFactory.getInstance()
                .getService(ServiceFactory.ServiceType.LECTURER);
        lectureTo.setId(lecturerId);
...
//                lecturerReqTo.getPicture().transfeTo(savePath.toFile());
//
...
//        em.getTransaction().begin();
//        try{
//            Lecturer newLecturer = mapper.map(lectureTo, Lecturer.class);
//            newLecturer.setId(lecturerId);
//            newLecturer.setPicture(currentLecturer.getPicture());
//            newLecturer.setLinkedIn(lectureTo.getLinkedin() != null ? new LinkedIn(newLecturer, lectureTo.getLinkedin()) : null);
